...
## 2026-05-21 - 운동·휴식 화면 양측 플로팅 패널 + 그라디언트 레이아웃

- 카메라 전체화면 배경 위에 좌우 그라디언트 패널이 떠있는 ProfileInputPage 동일 방식으로 전환.
- 좌측(40%): 운동명·세트정보·피드백 패널 / 우측(40%): 횟수·안정도·버튼 패널.
- 휴식(RestTimer) 화면도 동일하게 좌측(REST·완료) / 우측(카운트다운·다음 운동) 패널 적용.
- 코칭 오버레이 전체화면 방식 유지.

## 2026-05-21 - 운동·휴식 화면 60/40 좌우 분할 레이아웃 전환

- 세션 화면: 좌측 60% 카메라 피드, 우측 40% HUD 패널(운동명·횟수·피드백·버튼) 구조로 전환.
- 휴식(RestTimer) 화면도 동일하게 좌측 카메라, 우측 카운트다운 패널 레이아웃 적용.
- 코칭 오버레이(`phase="coaching"`)는 전체화면 오버레이 방식 그대로 유지.
- `position: absolute` 오버레이 방식 HUD 요소들을 정상 흐름(flex column)으로 전환.

## 2026-05-21 - 결과화면 운동 카드 갯수별 레이아웃 최적화

- `result-exercise-grid`에 `data-count` 속성을 추가해 렌더링 개수에 따라 열·카드 높이를 자동 조정합니다.
- 2개: 2열·300px, 3개: 3열·260px, 4개: 2열 2×2·260px, 1개: 3열·280px, 5개 이상: 기존 3열·228px.
- 상세 분석 섹션 제목을 `자세 분석과 PC3 근거` → `코칭 분석 리포트`로 변경했습니다.

## 2026-05-20 - 임시 버튼 API 미연결 데모 기능으로 수정

- 임시 버튼의 동작을 알림에서 다음 페이지 자동 이동으로 변경했습니다.
- 경로별 다음 단계 매핑을 추가해 프로필 선택 → 기준 촬영 → 루틴 → 세션 → 결과 → 기록 순서로 진행됩니다.
- API 연결 없이 화면 흐름을 테스트할 수 있는 데모 버튼으로 정리했습니다.
- `useLocation`을 활용해 현재 경로를 자동 감지합니다.

## 2026-05-20 - 임시 버튼 전 화면 공통 노출로 수정

- 임시 버튼 위치를 `AppShell`에서 `App` 전역으로 이동해 `프로필 선택` 화면 포함 모든 라우트에서 보이도록 수정했습니다.
- 하단 고정 입력 영역에 가려지지 않도록 `.temp-quick-button`의 z-index를 상향했습니다.
- `TEMP_UI_BUTTON_START/END` 경계 주석은 유지해 빠른 제거가 가능하도록 했습니다.

## 2026-05-20 - 전 화면 임시 UI 버튼 추가

- `src/components/AppShell.tsx`에 전 화면 공통으로 보이는 `임시 버튼`을 추가했습니다.
- 버튼 클릭 시 간단 동작 확인용 알림이 뜨도록 연결했습니다.
- 빠른 제거를 위해 코드에 `TEMP_UI_BUTTON_START`, `TEMP_UI_BUTTON_END` 주석 경계를 넣었습니다.
- `src/styles/layout.css`에 임시 버튼 전용 스타일(`.temp-quick-button`)을 추가했습니다.

## 2026-05-20 - 발표용 PC1 흐름 원고 전면 정리

- `PPT_FLOW_SUMMARY_KO.md`를 현재 코드 기준의 발표 원고 문서로 다시 작성했습니다.
  - 전체 구조, 라우트 흐름, 화면별 목적, PC3 API, 상태 데이터, 실패/차단 조건, 설치/연결 절차를 발표 순서로 정리했습니다.
  - 결과 화면 최신 구조인 `운동 완료`, `오늘 잘한 점`, `다음엔 이렇게`, `오늘 기록`, `자세 상세 보기`를 반영했습니다.
  - 화면 x API x 발표 슬라이드 매트릭스와 최종 체크리스트를 추가했습니다.
- `PC1_ONLY_FLOW_KO.md`를 PC1만 빠르게 설명하는 요약본으로 정상 한글 재작성했습니다.
- 기존 깨진 인코딩 문서 대신 PPT 제작자가 바로 옮길 수 있는 한국어 원고 형태로 정리했습니다.

## 2026-05-20 - PPT 문서에 빌드/설치/연결 절차 추가

- `PPT_FLOW_SUMMARY_KO.md`에 발표용 운영 절차를 추가했습니다.
	- 빌드 방법(`npm run build`, `npm run tauri -- build`, 배치 빌드)
	- 설치파일 생성/전달 순서
	- 설치 후 PC3 연결 점검 순서 및 성공 기준
- 핵심 주의사항(빌드 시점 `VITE_PC3_URL` 내장)을 문서에 명시했습니다.

## 2026-05-20 - PC1 전용 구조/흐름 요약 문서 추가

- 발표용 핵심만 남긴 `PC1_ONLY_FLOW_KO.md`를 추가했습니다.
- 범위를 PC1로 한정해 다음 항목만 정리했습니다.
	- 화면 흐름(프로필 -> 기준촬영 -> 루틴 -> 세션 -> 결과/기록)
	- PC1이 PC3에서 받아오는 핵심 값
	- 설치 후 연결 설정(`VITE_PC3_URL`)과 점검 순서

## 2026-05-20 - 결과 화면 코칭 리포트 카드형 적용

- 결과 화면을 `운동 완료`, `오늘 잘한 점`, `다음엔 이렇게`, `오늘 기록` 중심의 코칭 리포트 카드형으로 정리했습니다.
- 30일 누적 통계와 운동별 누적 횟수는 결과 화면에서 제거하고, 자세 오류/직전 비교/PC3·PC2 근거는 `자세 상세 보기` 접힘 영역으로 이동했습니다.
- 핵심 수치 4개 카드와 완료 히어로 전용 스타일을 추가해 운동 직후 사용자가 바로 읽기 쉽게 보강했습니다.

## 2026-05-20 - 배포 파일 생성, 발표 요약 문서, 루트 정리

- Tauri NSIS 설치파일을 생성해 루트에 `SmartMirror-PC1-Setup.exe`를 확정했습니다.
- 설치파일을 바탕화면에도 동일 이름으로 복사해 즉시 전달 가능한 상태로 맞췄습니다.
- 발표(PPT)용 구조/흐름 문서 `PPT_FLOW_SUMMARY_KO.md`를 추가했습니다.
- 요청 기준으로 불필요 산출물/임시 파일을 정리했습니다.
	- 삭제: `.codex-run`, `dist`, `node_modules`, 개발 로그, 미리보기 svg, 임시 txt, `api_check.py`
	- 유지: UI 실행 코드(`src`, `src-tauri`, `public`), PC3 계약/테스트 파일, `CHANGELOG.md`, `FLOW_CHANGES.md`, `SKILL.md` 포함 핵심 문서
- `.env`의 `VITE_PC3_URL` 기준 `GET /api/users/profiles` 실호출로 연결 상태(HTTP 200)를 확인했습니다.

## 2026-05-20 - 루틴 자동 준비/뒤로가기/결과 톤 마무리

- `/mode`에서 오늘 루틴이 없으면 수동 생성 버튼 없이 자동으로 루틴 생성을 요청하도록 정리했습니다.
- 루틴 미배정 상태 안내 문구를 자동 준비 흐름에 맞게 유지하고, 시작 버튼은 오늘 루틴 준비 완료 시에만 활성화되도록 유지했습니다.
- `/history` 날짜 이동을 `replace` 네비게이션으로 변경해 날짜 이동 이력이 브라우저 뒤로가기에 누적되지 않도록 수정했습니다.
- 프로필 선택 화면 상단의 `PC3에 저장된 프로필...` 안내 문구를 제거해 시작 화면을 더 간결하게 정리했습니다.
- 결과 화면 PANEL 5 제목을 `지난번 운동이랑 비교`로 변경하고, 패널 제목/자세 오류 요약 텍스트를 네온 시안 톤으로 보강했습니다.

## 2026-05-20 - 루틴 준비 상태 CTA 한 줄 추가

- `/mode`에서 오늘 루틴 미배정 상태일 때 버튼 아래에 다음 행동 안내 문구를 추가했습니다.
	- "지금 추천 루틴을 생성하면 오늘 루틴을 바로 시작할 수 있어요."
- 빈 상태에서 CTA가 더 눈에 띄도록 `today-panel--empty` 전용 색상 스타일을 보강했습니다.

## 2026-05-20 - 루틴 준비 화면 자연스러운 상태 UX 개선

- `/mode`에서 루틴 미배정 상태 전용 문구를 개선했습니다.
	- 오늘 선택 시: 추천 루틴 생성으로 바로 준비 가능하다는 안내로 변경
	- 일반 날짜 선택 시: 기존 "선택 날짜 루틴 없음" 안내 유지
- 추천 이유 카드의 빈 상태를 `대기 상태`, `다음 단계` 2개 카드로 분리해 다음 행동이 명확하게 보이도록 변경했습니다.
- 루틴 미배정 상태에서 버튼 텍스트를 상황형으로 조정했습니다.
	- 메인 버튼: `루틴 준비 후 시작`
	- 보조 버튼: `추천 루틴 생성` / 로딩 시 `루틴 준비 중`
- 루틴 미배정 상태 전용 레이아웃 클래스를 추가해 카드가 과하게 비어 보이던 문제를 완화했습니다.

## 2026-05-20 - 결과 화면 자세 오류 한글화 및 안정도 변화 설명 추가

- 자세 오류 요약 패널에서 `target recovering`, `multi person detected`가 영어로 노출되던 항목을 한글로 매핑했습니다.
- 패널 5 제목을 `전 세션 대비 개선도`에서 `직전 세션 비교 리포트`로 변경했습니다.
- 안정도 변화 값 옆에 계산 근거 문구를 추가했습니다.
	- 예: 이번 세션 0% - 직전 세션 76% 기준
	- 비교 데이터 부족 시 안내 문구 표시

## 2026-05-20 - 운동 중 target status 한글화 보강

- 운동 화면에서 `target recovering`, `multi person detected` 같은 공백 문자열 상태가 영어로 보이던 문제를 수정했습니다.
- 상태 문자열을 공백/하이픈/대소문자와 무관하게 정규화해 한글 라벨로 매핑하도록 보강했습니다.
- 경고 배지 판정도 정규화된 상태값 기준으로 동작하도록 수정했습니다.

## 2026-05-20 - 결과화면 패널 4/9 제거 및 8패널 재배치

- 요청에 따라 결과 패널 중 4번(측정 신뢰도/품질), 9번(baseline 대비 변화)을 제거했습니다.
- 남은 패널은 8개로 재번호화하고 시각 균형이 맞도록 재배치했습니다.
- `오늘 운동 요약`, `최근 30일 진행 지표`를 와이드 패널로 확장해 가독성을 높였습니다.
- 제거된 baseline 패널 관련 CSS도 정리했습니다.

## 2026-05-20 - 결과화면 10패널 전체 구성 추가

- Result 화면에 가이드 기준 10개 패널을 모두 배치했습니다.
- 추가 패널 범위:
	- 오늘 운동 요약
	- 자세 안정도 점수(게이지)
	- 좌우 밸런스
	...
	- 기준 촬영 대비 변화
	- 코칭 한 줄 행동 가이드
- 데이터가 없는 항목은 빈 상태 문구로 처리해 패널이 비어 보이지 않도록 정리했습니다.
- 모바일 화면에서는 일부 패널 레이아웃을 1열로 자동 전환하도록 스타일을 보강했습니다.

## 2026-05-20 - 결과화면 핵심 3패널 구현

- 결과 화면에 전 세션 대비 개선 배지를 추가했습니다.
	- 안정도 변화(%), 자세 오류 변화(개수)
- SESSION 카드에 좌우 밸런스 인디케이터를 추가했습니다.
	- 좌/우 카운트 차이값과 균형 상태(좋음/주의)
- COACHING 카드에 자세 오류 Top 3 패널을 추가했습니다.
	- 오류 목록이 없으면 "감지된 자세 오류가 없습니다"로 표시
- 관련 스타일을 `screens.css`에 반영했고 빌드 성공을 확인했습니다.

## 2026-05-20 - 결과 데이터 기반 패널 10종 정리 추가

- `RESULT_DATA_GUIDE_KO.md`에 현재 데이터로 구현 가능한 결과 패널 10가지를 추가했습니다.
- 각 패널별 목적과 표시 필드를 함께 정리해, 화면 설계 시 바로 참조할 수 있도록 문서화했습니다.
- 추가로 간단형 권장 배치(핵심 6패널)도 함께 정리했습니다.
...
- 기준 촬영 자동 흐름에서 3초 카운트다운을 제거하고, 후보 프레임을 PC3가 확인해 `valid` OK를 줄 때만 저장 완료로 진행하도록 바꿨습니다.
...
- 줄어든 루틴 홈 높이를 화면 본문 기준으로 복구하고, 좌우 패널이 같은 높이로 크게 보이도록 다시 맞췄습니다.
- 달력 날짜 카드에는 설명 문장 대신 `스쿼트와 점핑잭`, `스쿼트 외 2개` 같은 운동명 요약만 표시하도록 축약했습니다.
- 히스토리 날짜 메모가 있으면 루틴 달력 날짜 카드에 짧은 메모 미리보기를 함께 표시하도록 추가했습니다.
...
- `그날 요약` 패널을 요약 영역과 메모장 영역으로 나누고, 날짜별 메모를 PC1 화면용으로 저장해 루틴 달력에도 보이도록 연결했습니다.
- 히스토리의 몸무게 입력은 제거하고, `/mode`에서도 운동 전 몸무게 입력 카드를 제거해 루틴 확인 화면을 단순화했습니다.
...
- `이전` 버튼은 화면별 고정 fallback보다 실제 브라우저/app 바로 이전 기록으로 먼저 돌아가도록 정리했습니다.
