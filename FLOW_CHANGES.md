...
- 임시 버튼을 `AppShell` 내부가 아닌 `App` 전역에 배치해 모든 라우트에서 동일하게 보이도록 조정했습니다.
- 하단 고정 도크와 겹쳐 가려지지 않도록 버튼 z-index를 상향했습니다.
- 화면 흐름, API 호출, 상태 로직은 변경하지 않았습니다.

## 전 화면 임시 버튼 추가 (UI only)

이번 변경은 빠른 UI 확인을 위한 임시 조작점 추가입니다.

- `AppShell` 루트에 고정 `임시 버튼`을 추가해 어느 화면에서든 바로 눌러볼 수 있게 했습니다.
- 버튼 동작은 알림 1회 호출로 단순화했습니다.
- 제거 편의를 위해 `TEMP_UI_BUTTON_START/END` 경계 주석을 넣어 블록 단위로 쉽게 삭제할 수 있게 했습니다.
- 화면 흐름, 라우팅, API 호출, 상태 로직은 변경하지 않았습니다.

## 발표자료 운영 절차 보강 (빌드/설치/연결)

이번 변경은 발표 문서에서 기술 흐름뿐 아니라 실제 배포/연결 절차까지 설명 가능하도록 보강한 작업입니다.

- `PPT_FLOW_SUMMARY_KO.md`에 아래 섹션을 추가했습니다.
  - 빌드 방법
  - 앱(설치본) 생성 순서
  - 설치 후 PC3 연결 점검 방법
- 배포 시 혼선을 줄이기 위해 `VITE_PC3_URL` 내장 시점을 문서에 명확히 표기했습니다.

## 발표용 PC1 전용 문서 분리

이번 변경은 발표 자료 범위를 "PC1만"으로 좁히기 위한 문서 분리 작업입니다.

- `PC1_ONLY_FLOW_KO.md`를 추가해 다른 PC 내부 구조 설명을 최소화했습니다.
- 문서에는 아래만 남겼습니다.
  - PC1 화면 흐름
  - PC3와의 연결 지점(어디로 연결하는지)
  - PC1이 받는 값(무엇을 받아오는지)
  - 설치 후 다른 PC(PC3) 연결 방법

## 배포 준비 + 발표 전달용 정리 + 파일 경량화

이번 변경은 "설치파일 전달", "발표자료 반영", "작업 파일 정리"를 한 번에 마무리하는 운영 정리 작업입니다.

- 배포 파일
  - Tauri 빌드 산출물을 `SmartMirror-PC1-Setup.exe`로 루트에 확정했습니다.
  - 동일 파일을 바탕화면으로 복사해 설치 전달 동선을 단순화했습니다.
- 발표자료
  - `PPT_FLOW_SUMMARY_KO.md`를 추가해 발표용으로 바로 가져갈 수 있는 구조/흐름/계약/API/데모 시나리오를 정리했습니다.
- 파일 정리
  - 유지 대상: UI 실행 코드(`src`, `src-tauri`, `public`), PC3 계약 검증 파일(`test-api.http`), 핵심 문서(`CHANGELOG.md`, `FLOW_CHANGES.md`, `SKILL.md`, `README.md`).
  - 제거 대상: 로컬 빌드 산출물/캐시/임시 로그/미리보기 자산 등 실행과 계약에 직접 필요 없는 항목.
- 연결 확인
  - `.env`의 `VITE_PC3_URL` 기준 `GET /api/users/profiles` 호출로 PC3 연결 상태(200 OK)를 재확인했습니다.

## 루틴 자동 준비 + 뒤로가기 동선 정리 (Mode/History/Result)

이번 변경은 루틴 준비를 수동 클릭 없이 자연스럽게 진행하고, 결과 화면 문구/톤을 친근하게 맞추는 마무리 작업입니다.

- `/mode`
  - 오늘 루틴이 비어 있으면 `추천 루틴 생성` 버튼 없이 자동으로 생성 요청을 보내도록 변경했습니다.
  - 오류 재시도 시에도 동일한 자동 준비 흐름으로 다시 확인합니다.
- `/history`
  - 날짜 좌우 이동은 URL query를 갱신하되 `replace`를 사용해 히스토리 스택이 쌓이지 않도록 조정했습니다.
  - 따라서 뒤로가기 시 날짜 단위가 아니라 루틴 화면으로 자연스럽게 복귀됩니다.
- `/profile-select`
  - 시작 문맥에서 중복되던 PC3 설명 문구를 제거해 화면 집중도를 높였습니다.
- `/result`
  - PANEL 5 제목을 `지난번 운동이랑 비교`로 변경해 더 친근한 톤으로 조정했습니다.
  - 결과 패널 제목 및 자세 오류 리스트 텍스트를 네온 시안 계열로 보강해 화면 톤을 일치시켰습니다.
